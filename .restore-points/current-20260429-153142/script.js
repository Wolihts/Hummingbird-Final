...
function createNoteEntryFromKey(key) {
  return createNoteEntry(key.note, key.label, key);
}
...
function getSliderFill(value) {
  ...
  return `linear-gradient(to right, rgba(74, 222, 128, 0.92) 0%, rgba(190, 242, 100, 0.88) ${volume}%, rgba(255,255,255,0.12) ${volume}%, rgba(255,255,255,0.08) 100%)`;
}

function syncVolumeSlider(slider, value) {
  const volume = clampVolume(value);
  const valueLabel = slider.closest('.track-volume')?.querySelector('.track-volume-value');
  slider.value = String(volume);
  slider.style.background = getSliderFill(volume);
...
function getToolKeysForTrack(trackId) {
  ...
  const tool = track ? getResolvedTool(track.toolId, track.toneId) : null;
  return Array.isArray(tool?.keys) ? tool.keys : [];
}
...
function updateSelectedTone(toolId, toneId) {
  ...
  const matchingTracks = sessionTracks.filter((track) => track.toolId === toolId);
  if (matchingTracks.length > 0) {
    audioEngine.preloadInstruments(
      matchingTracks,
      (trackToolId, trackToneId) => getResolvedTool(trackToolId, trackToneId)
    ).catch(() => {});
  }
}
...
function renderInstrumentView(activeTool) {
  ...
  keyRange.textContent = `${resolvedTool.keyRange} / Focus: ${focusKeyText} / A-J row plays pads / Shift adds to step ${currentStep + 1}`;
  keysBoard.innerHTML = '';
  ...
  resolvedTool.keys.forEach((key, keyIndex) => {
    ...
    keyElement.addEventListener('click', async (event) => {
      ...
      await previewKey(resolvedTool, key);
    });
...
function addDefaultNoteToStep(trackId, stepIndex) {
  updateTrackStep(trackId, stepIndex, (track, currentStepEvents, index) => {
    const tool = getResolvedTool(track.toolId, track.toneId);
    if (!tool) {
      return getStepStack(currentStepEvents);
    }

    const nextStack = getStepStack(currentStepEvents);
    const defaultNote = tool.stepNotes[index % tool.stepNotes.length];
    const key = tool.keys.find((item) => item.note === defaultNote) || tool.keys[index % tool.keys.length];

    nextStack.push(key ? createNoteEntryFromKey(key) : createNoteEntry(defaultNote));
...
function updateStepNote(trackId, stepIndex, noteIndex, nextNote) {
  updateTrackStep(trackId, stepIndex, (_track, currentStepEvents) => {
    ...
    nextStack[noteIndex] = createNoteEntryFromKey(nextNote);
    return nextStack;
  });
}
...
function addSpecificNoteToStep(trackId, stepIndex, key) {
  updateTrackStep(trackId, stepIndex, (_track, currentStepEvents) => {
    const nextStack = getStepStack(currentStepEvents);
    nextStack.push(createNoteEntryFromKey(key));
    return nextStack;
  });
}
...
function placeNoteAtCurrentStep(tool, key) {
  ...
  sessionTracks = sessionTracks.map((track) => {
    ...
    currentStack.push(createNoteEntryFromKey(key));
...
function renderStepEditor() {
  ...
  tool.keys.forEach((key, keyIndex) => {
    stepEditorSelect.appendChild(createOption(String(keyIndex), `${key.label} (${key.note})`));
  });
  ...
  step.forEach((entry, noteIndex) => {
    ...
    tool.keys.forEach((key, keyIndex) => {
      const isSelected = key.label === entry.label || (
        key.note === entry.note
        && key.sampleInstrument === entry.sampleInstrument
      );
      select.appendChild(createOption(String(keyIndex), `${key.label} (${key.note})`, isSelected));
    });
    select.addEventListener('change', (event) => {
      const selectedKey = tool.keys[Number(event.currentTarget.value)];
      if (selectedKey) {
...
function createTrackRow(track, tool) {
    ...
    <div class="track-info">
      ...
      <div class="track-controls">
        ...
        <label class="track-volume">
          <input class="track-slider" type="range" min="0" max="100" step="1" value="${clampVolume(track.volume)}" aria-label="${track.name} volume" />
          <span class="track-volume-value">${clampVolume(track.volume)}%</span>
        </label>
      </div>
    </div>
  ...
  syncVolumeSlider(slider, track.volume);
  slider.addEventListener('input', (event) => {
    const volume = clampVolume(event.currentTarget.value);
    syncVolumeSlider(event.currentTarget, volume);
...
async function previewKey(tool, key) {
  try {
    setTransportStatus(`Loading ${tool.name} sample...`);
    await audioEngine.previewKey(tool, key);
...
function playCurrentStep(runId = playbackRunId) {
  ...
  tracksToPlay.forEach((track) => {
    ...
    audioEngine.triggerTrackStep(
      ...
      if (isPlaying && runId === playbackRunId) {
        setTransportStatus('Audio hiccup');
      }
    });
  });
...
stepEditorAddBtn.addEventListener('click', () => {
  ...
  const selectedKey = toolKeys[Number(stepEditorSelect.value)];
  if (selectedKey) {
